...
import { InsertUser, users, leads, InsertLead, applications, InsertApplication } from "../drizzle/schema";
...
export async function createApplication(app: InsertApplication): Promise<number | null> {
  const db = await getDb();
  if (!db) {
